...
{
#ifdef pdp11
#define MERGE1(x,mess2) {if(a1->x==0) a1->x = a2->x; else if(a2->x!=0 && a1->x!=a2->x) dclerr("inconsistent attributes",mess2); }
MERGE1(attype,"type");
MERGE1(attypep,"typep");
MERGE1(atprec,"prec");
MERGE1(atclass,"class");
MERGE1(atext,"text");
MERGE1(atcommon,"common");
MERGE1(atdim,"dim");
#else
...
#ifdef pdp11
#define MERGE(x,b,y) {if(v->y==0) v->y = a->x; else if(a->x!=0 && a->x!=v->y) dclerr("inconsistent attributes:","x"+2); }
#else
...
#ifdef pdp11
MERGE(atclass,"class",vclass);
#else
MERGE(atclass,vclass);
#endif
  }
...
switch(e->tag)
	...
	case TREPOP:
		if( ! isicon(e->leftp, &n) )
			...
			dclerr("nonconstant repetition factor",NULL);
			return(subs);
			}
		while(--n >= 0)
			{
